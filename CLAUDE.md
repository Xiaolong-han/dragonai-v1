...
│   │       ├── image.py          # 图像生成/编辑
│   │       ├── translation.py    # 翻译
│   │       ├── multimodal.py     # 多模态 (OCR/理解)
│   │       └── document.py       # 文档读取 (PDF/Word)
│   ├── api/                      # API 层
...
### 工具系统 (app/agents/tools/)

**内置工具** (ALL_TOOLS):
- `search_knowledge_base` - RAG 知识库检索 (`rag.py`)
- `web_search` - Tavily 网络搜索 (`web_search.py`)
- `ocr_document` - OCR 文档识别 (`multimodal.py`)
- `understand_image` - 图像理解 (`multimodal.py`)
- `generate_image` - 图像生成 (`image.py`)
- `edit_image` - 图像编辑 (`image.py`)
- `code_assist` - 代码助手 (`code.py`)
- `translate_text` - 文本翻译 (`translation.py`)
- `read_pdf` - 读取 PDF 文件 (`document.py`)
- `read_word` - 读取 Word 文件 (`document.py`)
