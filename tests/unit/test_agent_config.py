...
class TestAgentConfig:
    ...
    def test_agent_recursion_limit_exists(self):
        """测试 Agent 递归深度限制配置存在"""
        assert hasattr(settings, "agent_recursion_limit")
        assert settings.agent_recursion_limit > 0
        assert settings.agent_recursion_limit == 25
    ...
    def test_agent_config_types(self):
        """测试配置类型正确"""
        assert isinstance(settings.agent_recursion_limit, int)
    ...
    def test_agent_config_reasonable_values(self):
        """测试配置值合理"""
        assert 1 <= settings.agent_recursion_limit <= 100
