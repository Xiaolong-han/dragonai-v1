...
class TestAuthRegister:
    ...
    @pytest.mark.asyncio
    async def test_register_user_with_valid_data_returns_201(self, client, test_user_data):
        """正向测试: 使用有效数据注册用户应返回 201"""
    ...
    @pytest.mark.asyncio
    async def test_register_user_with_existing_username_returns_400(self, client, test_user_data):
        """异常测试: 使用已存在的用户名注册应返回 400"""
        # 首先注册一个用户
    ...
    @pytest.mark.asyncio
    async def test_register_user_with_existing_email_returns_400(self, client, test_user_data):
        """异常测试: 使用已存在的邮箱注册应返回 400"""
        # 首先注册一个用户
        await client.post("/api/v1/auth/register", json=test_user_data)
    ...
    @pytest.mark.asyncio
    async def test_register_user_without_email_returns_201(self, client):
